...
SELECT id, year, brand_name, name FROM models WHERE brand_name = 'Pontiac';
...
SELECT models.name, models.brand_name, headquarters FROM brands JOIN models ON (brands.name = models.brand_name) WHERE models.name = 'Mustang' AND models.brand_name = 'Ford';
...
SELECT id, name, founded, headquarters, discontinued FROM brands WHERE founded < 1903 ORDER BY founded;
...
SELECT m.name, m.brand_name, b.founded FROM brands AS b LEFT JOIN models AS m ON b.name = m.brand_name WHERE m.name IS NOT NULL;
...
SELECT brands.name, founded FROM brands LEFT JOIN models ON brands.name = models.brand_name WHERE brands.founded > 1955;
...
SELECT models.brand_name FROM models JOIN brands ON models.brand_name = brands.name GROUP BY models.brand_name HAVING COUNT(brands.name)>5;
...
SELECT models.name, models.year FROM models JOIN brands ON models.brand_name = brands.name WHERE models.year IN (SELECT founded FROM brands);
